...
module.exports = {
  name: 'info',
  description: 'Info sobre suas playlists.',
  userPrams: [],
  ...
  run: async (client, interaction) => {
      ...
      return interaction.editReply({
        embeds: [
          new MessageEmbed()
            ...
            .setDescription(`Você não tem uma playlist com o nome:  **${Name}**`),
        ],
      });
    } 
    ...
    const embed = new MessageEmbed()
      ...
      .setColor(client.embedColor)
      .setDescription(`**Nome da Playlist** ${pname} **Total de Musicas** \`${plist}\`\n\n${pages[page]}`);
    if (pages.length <= 1) {
      ...
      collector.on('collect', async (b) => {
        ...
        if (b.customId === 'Previous') {
          ...
          embed.setDescription(
            `**Nome da Playlist** ${pname} **Total de Musicas** \`${plist}\`\n\n${pages[page]}`,
          );
