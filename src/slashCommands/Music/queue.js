...
module.exports = {
  name: 'fila',
  description: 'Veja a fila de musicas.',
  userPrams: [],
  ...
  run: async (client, interaction) => {
    ...
    if (player.queue.length === '0' || !player.queue.length) {
      const embed = new MessageEmbed()
        ...
        .setDescription(
          `**Tocando Agora** [${player.current.title}](${player.current.uri}) • \`[ ${
      ...
      if (player.queue.length < 11) {
        const embed2 = new MessageEmbed()
          ...
          .setTitle(`${interaction.guild.name} Fila`);
        await interaction.editReply({
            embeds: [embed2],
          })
          .catch(() => {});
