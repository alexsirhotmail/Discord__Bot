...
module.exports = {
  name: 'play',
  description: 'Tocando alguma musica.',
  userPrams: [],
  ...
  run: async (client, interaction) => {
      ...
      return interaction.editReply({
        embeds: [
          new MessageEmbed()
            ...
            .setDescription(
              `Voce não tem permissão! please give me permission \`CONNECT\` or \`SPEAK\`.`,
            ),
        ],
      });
    ...
    if (!result.tracks.length) return interaction.editReply({ content: 'Não achei resultados' });
        ...
        ? {
          embeds: [
            new MessageEmbed()
              .setColor(client.embedColor)
              .setDescription(
                `${emojiplaylist} Busca ${tracks.length} de ${result.playlistName}`,
              ),
          ],
        }
        : {
          embeds: [
            new MessageEmbed()
              .setColor(client.embedColor)
              .setDescription(`${emojiaddsong} Busca [${tracks[0].title}](${tracks[0].uri})`),
          ],
        },
    );
  },
