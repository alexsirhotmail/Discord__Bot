  ...
  execute: async (message, args, client, prefix) => {
      ...
      return message.channel.send({
        embeds: [
          new MessageEmbed()
            ...
            .setDescription(`Nada para tocar agora.`),
        ],
      });
    ...
    if (player.queue.length === '0' || !player.queue.length) {
      const embed = new MessageEmbed()
        .setColor(client.embedColor)
        .setDescription(
          `**Tocando agora** [${player.current.title}](${player.current.uri}) • \`[ ${player.current.isStream ? '[**◉ LIVE**]' : convertTime(player.current.length)
      ...
      const queuedSongs = player.queue.map(
        (t, i) =>
          `\`[ ${++i} ]\` • [${t.title}](${t.uri}) • \`[ ${t.isStream ? '[**◉ LIVE**]' : convertTime(t.length)
          } ]\` • [${t.requester}]`,
      );
      ...
      if (player.queue.length < 11) {
        const embed = new MessageEmbed()
          .setColor(client.embedColor)
          .setDescription(
            `**Tocando agora**\n[${player.current.title}](${player.current.uri}) • \`[ ${player.current.isStream ? '[**◉ LIVE**]' : convertTime(player.current.length)
            } ]\` • [${player.current.requester}]\n\n**Musicas na Fila**\n${pages[page]}`,
          )
          .setFooter({
        ...
        collector.on('collect', async (button) => {
          if (button.customId === 'queue_cmd_but_1') {
            ...
            const embed3 = new MessageEmbed()
              ...
              .setTitle(`${message.guild.name} Busca`);
