    ...
    execute: async (message, args, client, prefix) => {
        try {
            ...
            if (args.length) {
                ...
                const title = player && player.queue && player.current ? `Tocando agora` : "Nothing is playing right now";
                const desc = player && player.queue && player.current ? `[${player.current.title}](${player.current.uri})` : null;
                const footer = {
                    text: player && player.queue && player.current ? `Solicitado por ${player.current.requester.tag}` : "",
